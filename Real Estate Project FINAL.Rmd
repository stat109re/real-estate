...
The size of the global professionally managed real estate market is about $8.5 trillion in 2017. Despite the sheer size of the market, projecting property pricing is still a challenging and very difficult task.

Usually, only socio-economic predictors are used to estimate the supply and demand change on the real estate market. However, the impact of online activity on real estate portals on annual price changes of real estate listings is still not well understood. In this paper, we are going to use socio-economic and listing data obtained from Realtor.com for fiscal year 2017 in order to provide an alternative pricing model.

This research is designed to help predict the year-on-year (YoY) price changes in 2017. The model would improve the understanding on the dynamic of the real estate market, which potentially could lead to meaningful business and social observations.

## Research Question

This research paper is to better understand the YoY changes of residential homes prices in the United States for 2017. Our statical analysis will focus on the impact of socio-economic indicators and online activity on YoY price changes of real estate online listings for single-family residential homes. The goal is to identify variables to predict the YoY price change in 2017 the most precisely. 

## Hypothesis

Under the economic conditions of 2017, we expect the number of affordable homes for sale to be the main driver of price growth. Our main hypothesis is higher price growth should occur in markets where inventory supply was low and dropping. Hence, inventory growth YoY and affordability should yield significant explanatory power of price. Another hypotheses states that growth in online searches should help tilt the balance into a sellers market and result in higher listing prices. Hence we are predicting that realtor.com page views per listing YoY would also be a significant predictor of price growth.
...
We now have 100 rows and 13 variables in our new dataset called listing100. We subset data to include largest 100 metropolitan areas only based on the knowledge that listing and economic indicators will be less noisy in the top 100 and that housing supply and demand dynamics will be more stable in larger cities.
...
Our dataset contains one categorical variables, regions, that include the Mid-West, North-East, South and West. The regions are allocated as follows:
...
### The median.Listing.Price.Yy, which is change in the price of a property as listed online (Realtor.com) has the following characteristics:
...
### The Active.Listing.Count.Yy, which is change in the number of listed properties online (Realtor.com) has the following characteristics:
...
### The Ldpviews.Per.Property.Yy, which is change in the the number of views of properties listed online (Realtor.com), has the following characteristics:
...
### The Median.Dom.Yy, which is change in the the number of days a property is listed online (Realtor.com), has the following characteristics:
...
### The Median.Listing.Price, which is the price of the proeprty listed online (Realtor.com), has the following characteristics:
...
### The Active.Listing.Count, which is number of properties listed online (Realtor.com), has the following characteristics:
...
### The Ldpviews.Per.Property, which is the number of views of properties listed online (Realtor.com), has the following characteristics:
...
### The Median.Dom, which is median number of days a property is listed online for (Realtor.com), has the following characteristics:
...
### The ActiveListingsper1000HH, which is the number of online properties listed per onbe thousand households (Realtor.com), has the following characteristics:
...
Now that we have decribed the varibles from the listing dataset, we can start performing various models.
...
We are going to analyze the listing data prior to running an analysis on both datasets together. This will enable us to understand the nature of the variables in the listing dataset first in order to highlight real-estate and lsiting-specific variables.
...
Model 3 shows that by adding the categorical variables region, the variables needed change. We now need the intercept, Avg..Active.Listing.Count.Yy, factor(Region)Northeast and factor(Region)West . The adjusted R-squared is higher than model 1 and the SSE is lower, which means that our overall prediction is better.


## MODEL 3

We are now going to run a stepwise regression on the prior model which now includes the regions:

```{r}
model.select(fit2, verbose = FALSE)
```

Now, let us look at one of the suggested above predictors individually - this would be Avg..Active.Listing.Count.Yy as it is statistically significant in all the above models:
...
The scatterplot and regression line look as follows:
...
From the Cook;s distance, we can identidy two major influential points that include 31 and 89. Let us list and identify the two longest on Cook's distance:
```{r}
listing100[c(31,89),1:4]
```

Let us remove Columbus, OH and Palm Bay, FL, as two problematic influential points. Our new listing name is going to be "listing98":
...
With a cleaner set of data, we can now run our 4th model on "listing98":
...
This 4th model has a slightly improved adjusted R-squared and a slightly better SSE, making it an overall better model.

Now that we have tested four distinct models on the listing dataset, we are going to make a partial conclusion prior to running a statistical analysis on the overall datatset that will include both the listing data and the economic data.

**Partial conclusion:** Because regions as part of geography are an important set of parameters for the real estate market, we are only going to consider models that include the catagorical variable. We can therefore conclude with confidence that our best model for the listing dataset is model #4.

The next step in our process is to add the economic variables onto the prior data and create a more refined model with all the data that we have collected from Realtor.com. After doing so, we are going to make initial conclusions on our findings in order to answer our theoretical question.


# STEP 2: ALL DATA ANALYSIS

We are now going to include our economic dataset in addition to the listing metrics to determine if our models can be improved. 
...
We are going to create a correlation heatmap for a preliminary analysis of dataset correlation:
...
In the bottom row of our correlation heatmap we observe one predictor variable in particular BUYPCT_yoy is potentially problematic because it is highly correlated with the reponse variable Avg..Median.Listing.Price.Yy. Upon further revision of how BUYPCT_yoy is defined in the data, we confirm this predictor should be excluded since Avg..Median.Listing.Price.Yy. is used in the formula for BUYPCT_yoy, and including it would render the model invalid. 

We also observe a few predictors are highly correlated amongst themselves, especially when the same metric is provided for the 2016 and 2017 years as exepected since estimates for these vary little over time. This happens with data for income (Income.2017 and Income.2016), jobs (End.Of.2016.Job and End.Of.2017.Job), unemployment (Unemployment.Rate.2016 and Unemployment.Rate.2017) and affordability (Buy.Pct.2016 and Buy.Pct.2017). Before we exclude those, we run further diagnostics to see how to treat these.
...
This fifth model that includes all our variables indicates that six variables are needed in the model. Before any further consideration, we are going to run some diagnostics, particularly as it relates to heteroskedasticity and multicollinearity:

```{r}
ncvTest(fit5)
```

There is no heteroskedasticity in this 5th model as shown by the p-value greater than 0.05. Let us try to determnine is there is multicollinearity in the model:

```{r}
vif(fit5)
```

Many predictors show VIFs above 10, pointing to mutli-collinearity in our dataset and confirming the problematic associations we uncovered earlier in the correlation heatmap. We will exclude the 2017 estimates for those household, job, unemployment and affordability measures and leave the 2016 values only.
...
```
Checking VIFs again, End.Of.2017.Household is still the highest above 10. We remove and refit. Running VIFs again shows all are now under 10, as shown below:
...
We are going to run a stepwise regression on the prior model to indicate which variables are needed. We can then compare the SSE and adjusted R-squared:
...
In comparison to all our prior models, model 6 has the highest adjusted R-squared and the lowest SSE, making it our most reliable model so far. We run more diagnostics to ensure the data we are modeling does not have collinearity and heteroskedasticity:
...
```
 Cook's distance only highlights two points that would be considered influential, 89 and 8.
...
The above shapiro test for normality shows that our data is normaly distributed.
...
The above ncvTest confirms that there is no heteroskedasticity in the model, and the RESET test indicates that no transformation is needed because of the p-value above 0,05.
...
Next we use the step function to choose a model by AIC in a Stepwise Algorithm as an alternative method. This model has an even higher adjusted R-squared and the lowest SSE compared to earlier models. Diagnostics ran on the model also indicate no issues:
...
So far we have assessed the models based on the full set but don't know how they behave with test data. Next, we split our dataset into 'Train' and 'Test' subsets, and run procedure on our best models to obtain Test RMSEs and determine our final model. To achieve that, our code is as follows:
...
The RMSE on the test data for model 6 is 0.0278. We will now run the same step for model 7.
...
The RMSE for the test data for model 7 is 0.0306.
...
The RMSE for the test data for model 8 is 0.0411.
...
The RMSE for the test data for model 9 is 0.0350.
...
The RMSE for the test data for model 9 is 0.0323.
...
The RMSE for the test data for model 10 is 0.0319.
...
**Partial Conclusion** The lowest RMSE, as stated above, if for model 6 with 0.02781. To ensure that model 6 is our best and final model, we are going to train and test the data on loops for 1,000 times for all five models and compare the results one last time.

---

##Finally, repeat the Train/Test evaluations a 1,000 times:
...
#model 8 - Ridge Regression - MISSING
   ...
   fits = x.test%*%coef(fit8_ridge_train)
   rmse_fit8[i] <- sqrt(mean((fits-test.data$Avg..Median.Listing.Price.Yy)^2))
#model 9 - All Possible Regressions - MISSING
   train.data.shift<- train.data
   train.data.shift$Avg..Median.Listing.Price.Yy=train.data$Avg..Median.Listing.Price.Yy+0.03
   ...
   fit9=lm(Avg..Median.Listing.Price.Yy ~ Region + Avg..Active.Listing.Count.Yy +Avg..Ldpviews.Per.Property.Yy +
           Avg..Median.Dom.Yy   +  Avg..Median.Listing.Price +    Avg..Active.Listing.Count  +  Avg..Ldpviews.Per.Property 
            +  Avg..Median.Dom  + Avg.ActiveListingsper1000HH    +           End.Of.2016.Job        +           Income.2016 
     +  Unemployment.Rate.2016          +        Buy.Pct.2016            +            HH_yoy      +                 JOB_yoy 
                   +   INC_yoy           +          UNEMP_yoy     +      Home.Ownership.2017       +           OwnOccHH2017 
   ...
   rmse_fit9[i] <- sqrt(mean((predict(fit9,newdata=test.data.shift)-test.data.shift$Avg..Median.Listing.Price.Yy)^2))

#model 10  - Lasso  - MISSING
   ...
   rmse_fit10[i] <- sqrt(mean((fits-test.data$Avg..Median.Listing.Price.Yy)^2))
...
Our analysis highlighted that, unlike what was thought initially, region (our only categorical variable in the set) does not affect the average median listing price year over year, nor does the number of online views onto Realtor.com website. Our initial assumption was that the greater the number of online views, the higher the average median listing price year over year. In fact, we proved that a fair amount of viewership is simply related to looking at real estate in a passive manner as opposed to looking at real estate solely prior to a purchase. From a practical standpoint, this implies that anyone with access to the data (from public sources for instance) can build a robust model about price prediction from one year to another and that this is not an analysis limited to companies that can compute the amount of viewership onto their websites.
